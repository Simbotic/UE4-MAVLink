...
void FMavlinkMsg_scaled_imu::Serialize(uint8 systemId, uint8 componentId, __mavlink_message& msg, uint16& packSize)
{
    packSize = mavlink_msg_scaled_imu_pack(systemId, componentId, &msg,  time_boot_ms, xacc, yacc, zacc, xgyro, ygyro, zgyro, xmag, ymag, zmag);
}

void FMavlinkMsg_scaled_imu::Serialize(uint8 systemId, uint8 componentId, TSharedRef<TArray<uint8>, ESPMode::ThreadSafe>& buffer)
{
    uint16 size;
...
void FMavlinkMsg_scaled_imu::Deserialize(const mavlink_message_t& msg)
{
        ...
        FMemory::Memset(this, 0, MAVLINK_MSG_ID_SCALED_IMU_LEN);

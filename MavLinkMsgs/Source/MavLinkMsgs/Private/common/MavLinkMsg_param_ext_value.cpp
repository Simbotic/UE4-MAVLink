...
void FMavlinkMsg_param_ext_value::Deserialize(const mavlink_message_t& msg)
{
        ...
        param_count = mavlink_msg_param_ext_value_get_param_count(msg);
        param_index = mavlink_msg_param_ext_value_get_param_index(msg);
        param_id = mavlink_msg_param_ext_value_get_param_id(msg, param_ext_value->param_id);
        param_value = mavlink_msg_param_ext_value_get_param_value(msg, param_ext_value->param_value);
        param_type = mavlink_msg_param_ext_value_get_param_type(msg);
    
    #else
        uint8_t len = msg.len < MAVLINK_MSG_ID_PARAM_EXT_VALUE_LEN? msg.len : MAVLINK_MSG_ID_PARAM_EXT_VALUE_LEN;
        FMemory::Memset(this, 0, MAVLINK_MSG_ID_PARAM_EXT_VALUE_LEN);
